...
class RUF(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.load_pretrained_body:
            from ..build_model import model_cfg2model
            model = model_cfg2model(cfg.pretrained_body)

            model.load_state_dict(torch.load(os.path.join(cfg.pretrained_body.pretrained_path, "model.pt")))
    ...
    def decode(self, middle, skips, sizes, original_size):
        ...
        dec_outs[3] = self.decoder[3](dec_outs[2], sizes[3], skips[3])  # 330
